...
const SearchForm = ({ title, relationshipSearch, width = "", defaultValues, UFCClear, buttonTitle, handleSubmitUSCensus1830, nearestResidenceDate }) => {
  const formValidate = (_values) => {
    let error = {
      invalid: "Invalid",
    };
    let relations = _values.rs?.filter(
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    ...
    handleSubmitUSCensus1830(formValue, { setSubmitting });
  };
  ...
  const get1830LifeEvents = async (val, setFieldValue) => {
    await apiRequest(GET, `persons/${val.id}/alllifeevents`).then((res) => {
      const LifeEventsData1830 = res.data;
      setBestResidence(LifeEventsData1830 , nearestResidenceDate , setFieldValue , "r" , "Residence",false)
    })
  }
                    ...
                    <Field
                      ...
                      selectPeople={(val) => {
                        if (val?.givenName) {
                          ...
                          get1830LifeEvents(val, setFieldValue)
                        }
                      }}
