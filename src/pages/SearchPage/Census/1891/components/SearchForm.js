...
const SearchForm = ({ title, relationshipSearch, width = "", defaultValues, clear1891, buttonTitle, handleSubmitUKCensus1891, nearestResidenceDate }) => {
  const formValidate = (values1891) => {
    let error = {
      invaild: "Inavild",
    };
    let relations = values1891.rs?.filter((y) => (y?.f && y?.f?.trim() !== "") || (y?.l && y?.l?.trim() !== ""));
    if ((values1891.fm.t === "" || values1891.fm.t?.name?.trim() === "") && (values1891.ln.t === "" || values1891.ln.t?.trim() === "") && (values1891.RSPlace.name === "" || values1891.RSPlace.name?.trim() === "" || values1891.RSPlace.name === undefined) && (values1891.BirthPlace.name === "" || values1891.BirthPlace.name?.trim() === "" || values1891.BirthPlace.name === undefined) && values1891.g === "" && values1891.ms === "" && values1891.rh === "" && !relations.length) {
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    ...
    handleSubmitUKCensus1891(formValue, { setSubmitting });
  };

  const defaultTypeCensusSearch1891 = typeSearchDefaulUsCensus1901();
  const matchInputField1891 = (setFieldValue, values) => {
    let html1891 = null;
    html1891 = (
  ...
  const get1891LifeEvents = async (val, setFieldValue) => {
    await apiRequest(GET, `persons/${val.id}/alllifeevents`).then((res) => {
      const LifeEventsData1891 = res.data;
      LifeEventsData1891?.map((Data1891) => {
        if (Data1891.type === "Birth") {
          setFieldValue("BirthPlace.name", Data1891.location.Location || "");
          setFieldValue("BirthPlace.id", Data1891.location.LocationId);
          setFieldValue("b.y.y", Data1891.date.Date?.YearValue);
          setFieldValue("b.l.l", Data1891.location.Location);
          setFieldValue("b.l.s", "1");
          setFieldValue("b.li.s", "4");
        }
      });
      const Birth = LifeEventsData1891?.filter((data) => data.type === "Birth" && data?.date?.Date.YearValue !== null)?.map((data) => {
        return data;
      });
      const Residence = LifeEventsData1891?.filter((data) => data.type === "Residence" && data?.date?.Date.YearValue !== null)?.map((data) => {
        return data;
      });
  ...
  const SetEmptyFields1891 = (setFieldValue) => {
    setFieldValue("RSPlace.id", "");
    setFieldValue("RSPlace.name", "");
    setFieldValue("BirthPlace.id", "");
    setFieldValue("BirthPlace.name", "");
  };
  const get1891Parents = (data1891, index, setFieldValue) => {
    if (data1891.relation === "parent" && data1891.gender === "M") {
      setFieldValue(`rs[${index}].f`, data1891.givenName);
      setFieldValue(`rs[${index}].l`, data1891.surname);
      setFieldValue(`rs[${index}].r`, "Father");
    }

    if (data1891.relation === "parent" && data1891.gender === "F") {
      setFieldValue(`rs[${index}].f`, data1891.givenName);
      setFieldValue(`rs[${index}].l`, data1891.surname);
      setFieldValue(`rs[${index}].r`, "Mother");
    }
  };
  const getFamilyRelationships = (family1891, setFieldValue) => {
    if (family1891) {
      family1891.map((data1891, index) => {
        get1891Parents(data1891, index, setFieldValue);

        if (data1891.relation === "partner") {
          setFieldValue(`rs[${index}].l`, data1891.surname);
          setFieldValue(`rs[${index}].f`, data1891.givenName);
          setFieldValue(`rs[${index}].r`, "Spouse");
        }
        if (data1891.relation === "child") {
          setFieldValue(`rs[${index}].f`, data1891.givenName);
          setFieldValue(`rs[${index}].r`, "Child");
          setFieldValue(`rs[${index}].l`, data1891.surname);
        }
        if (data1891.relation === "sibling") {
  ...
  const get1891PersonRelationShips = async (val, setFieldValue) => {
    await apiRequest(GET, `Persons/relationships?treeId=${val.treeId}&personId=${val.id}`).then((res) => {
      ...
      const family1891 = [...updatedParentsAndSiblings, ...spousesAndChildren];
      getFamilyRelationships(family1891, setFieldValue);
    });
  };
  return (
    <>
            ...
            <Form className="w-full">
              <div className="flex flex-wrap -mx-2 md:mb-2.5">
                <div className={`w-full px-2 mb-2.5 ${width}`}>
                  <label className="block text-gray-6 text-sm mb-1" htmlFor="grid-1891-field">
                  ...
                  <div className="relative">
                    <Field
                      ...
                      selectPeople={(val) => {
                        if (val?.givenName) {
                          SetEmptyFields1891(setFieldValue);
                          setFieldValue("rs", []);
                          setFieldValue("ln.t", val?.surname || "");
                          get1891LifeEvents(val, setFieldValue);
                          get1891PersonRelationShips(val, setFieldValue);
                        }
                      }}
                ...
                <div className={`w-full px-2 mb-2.5 ${width} `}>{inputField(tr(t, "search.ww1.form.lname"), "ln", values, setFieldValue, handleChange, defaultTypeCensusSearch1891, t)}</div>
              </div>
              ...
              <>
                ...
                <div className="pt-4 mb-2 sm:flex justify-between w-full md:pt-7">
                  {matchInputField1891(setFieldValue, values)}
