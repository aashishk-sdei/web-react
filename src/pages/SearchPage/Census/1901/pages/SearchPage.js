...
import { encodeDataToURL, getPageSize, formDataTrim, getUsCensus1901DefaultValue, USFederal1901PK } from "../../../../../utils";
import { tr } from "../../../../../components/utils";
import UsCensus1901SearchForm from "../components/SearchForm";
import { getUsCensus1901DropdownList } from "../../../../../redux/actions/UsCensus1901";
...
const SearchPage = () => {
  ...
  useEffect(() => {
    ...
    dispatch(getContentCatalog({ partitionKey: USFederal1901PK }));
    return () => {
      dispatch(updateContentCatalog());
    };
  }, [dispatch]);
  const handleSubmitUsCensus1901 = (values, { setSubmitting }) => {
    setSubmitting(false);
    if (!values?.b?.l?.l) {
      delete values.b.l;
    }
    ...
    history.push("/search/1901-united-kingdom-census/result?" + urlQuery);
  };
  return (
    <div className="main-container">
      <div className="pt-12 md:pt-14 h-full md:bg-transparent bg-white">
        ...
        <div className="bg-white tw-search-form-card md:rounded-2xl p-6 md:p-10 md:shadow-search-modal md:mt-10 md:mb-8 max-w-src-modal-w w-full mx-auto relative">
          <UsCensus1901SearchForm title={contentCatalog?.collectionTitle} relationshipSearch={contentCatalog?.addRelationshipSearch} width={"md:w-1/2"} nearestResidenceDate={contentCatalog?.nearestResidenceDate} defaultValues={defaultValues} handleSubmitUsCensus1901={handleSubmitUsCensus1901} clear={true} buttonTitle={tr(t, "search.ww1.form.btn.search")} />
        </div>
        <MetaData />
      </div>
    </div>
