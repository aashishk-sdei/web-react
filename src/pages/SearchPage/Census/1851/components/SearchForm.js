...
const SearchForm = ({ title, relationshipSearch, width = "", defaultValues, clear1851, buttonTitle, handleSubmitUKCensus1851, nearestResidenceDate }) => {
  const formValidate = (values1851) => {
    let error = {
      invaild: "Inavild",
    };
    let relations = values1851.rs?.filter((y) => (y?.f && y?.f?.trim() !== "") || (y?.l && y?.l?.trim() !== ""));
    if ((values1851.fm.t === "" || values1851.fm.t?.name?.trim() === "") && (values1851.ln.t === "" || values1851.ln.t?.trim() === "") && (values1851.RSPlace.name === "" || values1851.RSPlace.name?.trim() === "" || values1851.RSPlace.name === undefined) && (values1851.BirthPlace.name === "" || values1851.BirthPlace.name?.trim() === "" || values1851.BirthPlace.name === undefined) && values1851.g === "" && values1851.ms === "" && values1851.rh === "" && !relations.length) {
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    ...
    handleSubmitUKCensus1851(formValue, { setSubmitting });
  };

  const defaultTypeCensusSearch1851 = typeSearchDefaulUsCensus1901();
  const matchInputField1851 = (setFieldValue, values) => {
    let html1851 = null;
    html1851 = (
  ...
  const getSingleBeforeConditions = (setFieldValue, beforeYearData, afterYearData, data) => {
  ...
  const getBeforeConditions = (beforeYearData, afterYearData, data, setFieldValue) => {
    getSingleBeforeConditions(setFieldValue, beforeYearData, afterYearData, data);
    if (beforeYearData.length > 1) {
  ...
  const getAfterConditions = (setFieldValue, afterYearData, data) => {
  ...
  const getSingleYearResidenceConditions = (setFieldValue, residenceData) => {
  ...
  const getMultipleYearResidenceConditions = (setFieldValue, residenceData) => {
    if (residenceData.length > 1) {
      ...
      residenceData?.forEach((data) => {
        if (data?.date?.Date.YearValue < nearestResidenceDate.yearValue) {
          ...
          getAfterConditions(setFieldValue, afterYearData, data);
        }
      });
    }
  };
  const get1851LifeEvents = async (val, setFieldValue) => {
    await apiRequest(GET, `persons/${val.id}/alllifeevents`).then((res) => {
      const LifeEventsData1851 = res.data;
      LifeEventsData1851?.map((Data1851) => {
        if (Data1851.type === "Birth") {
          setFieldValue("BirthPlace.name", Data1851.location.Location || "");
          setFieldValue("BirthPlace.id", Data1851.location.LocationId);
          setFieldValue("b.y.y", Data1851.date.Date?.YearValue);
          setFieldValue("b.l.l", Data1851.location.Location);
          setFieldValue("b.l.s", "1");
          setFieldValue("b.li.s", "4");
        }
      });
      const Birth = LifeEventsData1851?.filter((data) => data.type === "Birth" && data?.date?.Date.YearValue !== null)?.map((data) => {
        return data;
      });
      const Residence = LifeEventsData1851?.filter((data) => data.type === "Residence" && data?.date?.Date.YearValue !== null)?.map((data) => {
        return data;
      });
      const residenceData = [...Birth, ...Residence];
      getSingleYearResidenceConditions(setFieldValue, residenceData);
      getMultipleYearResidenceConditions(setFieldValue, residenceData);
    });
  };
  const SetEmptyFields1851 = (setFieldValue) => {
    setFieldValue("RSPlace.id", "");
    setFieldValue("RSPlace.name", "");
    setFieldValue("BirthPlace.id", "");
    setFieldValue("BirthPlace.name", "");
  };
  const get1851Parents = (data1851, index, setFieldValue) => {
    if (data1851.relation === "parent" && data1851.gender === "M") {
      setFieldValue(`rs[${index}].f`, data1851.givenName);
      setFieldValue(`rs[${index}].l`, data1851.surname);
      setFieldValue(`rs[${index}].r`, "Father");
    }

    if (data1851.relation === "parent" && data1851.gender === "F") {
      setFieldValue(`rs[${index}].f`, data1851.givenName);
      setFieldValue(`rs[${index}].l`, data1851.surname);
      setFieldValue(`rs[${index}].r`, "Mother");
    }
  };
  const getFamilyRelationships = (family1851, setFieldValue) => {
    if (family1851) {
      family1851.map((data1851, index) => {
        get1851Parents(data1851, index, setFieldValue);

        if (data1851.relation === "partner") {
          setFieldValue(`rs[${index}].l`, data1851.surname);
          setFieldValue(`rs[${index}].f`, data1851.givenName);
          setFieldValue(`rs[${index}].r`, "Spouse");
        }
        if (data1851.relation === "child") {
          setFieldValue(`rs[${index}].f`, data1851.givenName);
          setFieldValue(`rs[${index}].r`, "Child");
          setFieldValue(`rs[${index}].l`, data1851.surname);
        }
        if (data1851.relation === "sibling") {
  ...
  const get1851PersonRelationShips = async (val, setFieldValue) => {
    await apiRequest(GET, `Persons/relationships?treeId=${val.treeId}&personId=${val.id}`).then((res) => {
      ...
      const family1851 = [...updatedParentsAndSiblings, ...spousesAndChildren];
      getFamilyRelationships(family1851, setFieldValue);
    });
  };
  return (
    <>
            ...
            <Form className="w-full">
              <div className="flex flex-wrap -mx-2 md:mb-2.5">
                <div className={`w-full px-2 mb-2.5 ${width}`}>
                  <label className="block text-gray-6 text-sm mb-1" htmlFor="grid-1851-field">
                  ...
                  <div className="relative">
                    <Field
                      ...
                      selectPeople={(val) => {
                        if (val?.givenName) {
                          SetEmptyFields1851(setFieldValue);
                          setFieldValue("rs", []);
                          setFieldValue("ln.t", val?.surname || "");
                          get1851LifeEvents(val, setFieldValue);
                          get1851PersonRelationShips(val, setFieldValue);
                        }
                      }}
                ...
                <div className={`w-full px-2 mb-2.5 ${width} `}>{inputField(tr(t, "search.ww1.form.lname"), "ln", values, setFieldValue, handleChange, defaultTypeCensusSearch1851, t)}</div>
              </div>
              <>
                <div className="flex flex-wrap md:mb-2.5 -mx-2">
                  <div className={`px-2 mb-2.5 br-field-md w-full`}>{locationField("Residence", "r", "RSPlace", values, setFieldValue)}</div>
                </div>
                <div className="flex flex-wrap md:mb-2.5 -mx-2">
                  <div className={`px-2 mb-2.5 br-field-md w-full `}>{locationField("Birthplace", "b", "BirthPlace", values, setFieldValue)}</div>
                </div>
                <div className="flex md:mb-2.5 flex-wrap -mx-2 md:max-w-lg">
                ...
                <div className="pt-4 mb-2 sm:flex justify-between w-full md:pt-7">
                  {matchInputField1851(setFieldValue, values)}
