...
import { submitGermanForm, getGermanDropdownList, clearGermanFormQuery } from "../../../../../redux/actions/germanToAmerica";
...
import { getGermanToAmericaDefaultValue, pageRecordsCountfn, getButtonTitle, getFirstName, decodeDataToURL, encodeDataToURL, getPageSize, mergeDeep, formDataTrim, BG_GRAY_1 } from "../../../../../utils";
...
const setFormData = (setValues, _values, ResObj, birthObj, PDepartObj, iDestObj) => {
  setValues({
...
const SearchPageList = () => {
  ...
  const { germanList, loading, pageLoading, error , fuzzyMatch } = useSelector((state) => state.germanToAmerica);


  const handleSubmitGermanToAmerica = (formValuesGA) => {
    newRequest.current = true;
    setShowModal(false);
    genarateUrl({...formValuesGA , f : false}, history);
  };
  ...
  const getGermanHtmlData = () => {
    if (loading) {
      ...
      return germanList.length === 0 ? <Norecords searchResult={germanList} isLoading={false} firstName={getFirstName(values)} /> : <GermanTable data={germanList} maskingFields={maskingFields} error={error} loading={loading} isPageLoad={isPageLoad} tableColWidth={tableColWidth} tableColTotal={tableColTotal} tableSize={getTableSize} handleTableColWidth={handleTableColWidth} handleTableColTotal={handleTableColTotal} isNewSearch={true} />;
    }
  };
  return (
    <>
      <div className="page-wrap ww-results-wrap">
          ...
          <div className="container mx-auto">
                ...
                {germanPageRecords !== 0 && (
                  <p>
                    <Typography fontFamily="primaryFont" size={14} text="secondary" weight="light">
                      {PaginationResult(t, germanPageRecords, values?.ps, values?.pn, totalRecords)}
                    </Typography>{" "}
                  </p>
                )}
              </div>
            ...
            <div className="ww-grid">
              <div className="relative ww-col-right">
                <div className="flex">
                  <div className="table-content bg-white sm:rounded-lg sm:shadow text-left">{getGermanHtmlData()}</div>
                </div>
                <div className="px-3 sml:px-0 relative -top-4 sml:top-0 pb-2 sml:pb-5">
                  <TWPaginationComponent currentPage={parseInt(values?.pn || 0)} totalRecords={totalRecords} limitPerPage={parseInt(values?.ps || 20)} getList={getGermanListPage} changeLimit={changeLimit} tableSize={tableSize} />
                </div>
              </div>
              <div
              ...
              >
                <div className="bg-white sm:rounded-lg shadow p-3 md:py-5 md:px-6">
                  ...
                  <GermanToAmericaRefineSearch width={""} handleSubmitGermanToAmerica={handleSubmitGermanToAmerica} germanDefaultValues={values} handleShowModal={handleShowGermanModal} buttonTitle="Update" />
                </div>
              </div>
            </div>
          </div>
        ...
        {/*Modal Tailwind Form*/}
        <TailwindModal title={contentCatalog?.collectionTitle} showClose={true} content={<GermanToAmericaSearchForm inputWidth={""} nearestResidenceDate = {contentCatalog?.nearestResidenceDate} width={"sm:w-1/2"} handleSubmitGermanToAmerica={handleSubmitGermanToAmerica} defaultValues={valuesNew} germanClear={false} buttonTitle={buttonTitle} />} showModal={showModal} setShowModal={setShowModal} isPropagation={false} />
      </div>
    </>
  );
};
